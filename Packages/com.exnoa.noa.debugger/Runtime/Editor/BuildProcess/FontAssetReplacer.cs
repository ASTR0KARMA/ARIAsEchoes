        ...
        ReplaceState _ReplaceFontAsset(Transform target, NoaDebuggerSettings settings)
        {
           ReplaceState replaceState = ReplaceState.NoReplace;
            ...
            if (isOutermost)
            {
                ...
                var addedComponents = PrefabUtility.GetAddedComponents(target.gameObject);
                foreach(AddedComponent component in addedComponents)
                {
                    if (component.instanceComponent is NoaDebuggerText)
                    {
                        NoaDebuggerText textMeshPro = component.instanceComponent as NoaDebuggerText;
