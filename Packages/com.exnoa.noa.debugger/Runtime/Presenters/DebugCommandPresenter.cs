...
namespace NoaDebugger.DebugCommand
{
    ...
    sealed class DebugCommandPresenter : NoaDebuggerToolBase, INoaDebuggerTool, INoaDebuggerFloatingTool
    {
        ...
        public static bool IsAutoRefresh
        {
            get
            {
                if (DebugCommandPresenter._instance == null ||
                    DebugCommandPresenter._instance._model == null)
                {
                    return false;
                }
                return DebugCommandPresenter._instance._model.IsAutoRefresh;
            }
            set
            {
                if (DebugCommandPresenter._instance == null ||
                ...
                DebugCommandPresenter._instance._model.UpdateAutoRefresh(value);
                DebugCommandPresenter._instance._UpdateView(CommandViewUpdateTarget.Buttons);
            }
        }

        public static event Action OnRefreshProperty;
        ...
        public void Init()
        {
            ...
            _model.OnAutoRefreshStateChanged = _OnAutoRefreshStateChanged;
            ...
            SettingsEventModel.OnUpdateCommandSettings -= _OnUpdateDebugCommandSettings;
            SettingsEventModel.OnUpdateCommandSettings += _OnUpdateDebugCommandSettings;
        }
        ...
        void _InitView(DebugCommandView view)
        {
            ...
            view.OnFloatingSelectAllButton += _OnSelectAllGroupForFloatingWindow;
        }
        ...
        GroupPanelInfo[] _CreateGroups(string category, Dictionary<string, CommandGroupData> groupDic)
        {
            ...
            foreach (var group in groupDic.OrderBy(x => x.Value._order ?? Int32.MaxValue))
            {
                ...
                GroupCollapseToggleInfo collapseToggleInfo = new GroupCollapseToggleInfo(
                    group.Value._isCollapsed,
                    _OnSelectGroupForCollapse,
                    _OnSelectAllGroupForCollapse);
        ...
        {
            _model.UpdateAutoRefresh(!_model.IsAutoRefresh);
        }

        void _OnAutoRefreshStateChanged(bool isAutoRefresh)
        {
            var linker = new DebugCommandViewLinker
            {
                _isAutoRefresh = _model.IsAutoRefresh,
                _updateTarget = CommandViewUpdateTarget.Buttons
            };
        ...
        void _OnSelectGroupForCollapse(string group, bool isOn)
        {
            ...
            _model.UpdateCollapseStatus(category, group, isOn);
            _model.SaveCollapseStatus();
        }

        void _OnSelectAllGroupForCollapse(bool isOn)
        {
            int index = _GetCategoryIndex();
            string category = _model.CategoryNames[index];
            var groups = _model.GetGroupsForCategory(category);
            ...
            {
                group.Value._isCollapsed = isOn;
                _model.UpdateCollapseStatus(category, group.Key, isOn);
            }

            _model.SaveCollapseStatus();
            _UpdateView(CommandViewUpdateTarget.CommandPanel);
        }
        ...
        void _OnSelectAllGroupForFloatingWindow(bool isOn)
        {
            int index = _GetCategoryIndex();
            string category = _model.CategoryNames[index];
            ...
            if (isOn)
            {
                ...
                _floatingRemoveGroups[category] = Array.Empty<string>();
            }
            else
            {
                string[] allGroupNames = _model.GetGroupsForCategory(category).Keys.ToArray();
                if (_floatingRemoveGroups.ContainsKey(category))
                {
                    _floatingRemoveGroups[category] = allGroupNames;
                }
                else
                {
                    _floatingRemoveGroups.Add(category, allGroupNames);
                }
            }
        ...
        void _OnUpdateDebugCommandSettings()
        {
            _ApplySettings();
            _UpdateView(CommandViewUpdateTarget.CommandPanel);
        }
        ...
        public static void RefreshProperty()
        {
            ...
            DebugCommandPresenter.OnRefreshProperty?.Invoke();
        }
        ...
        void _SaveFilter()
        {
            ...
            filters._groupFilterJson = _floatingRemoveGroups
                ...
                .Select(target =>
                {
                    ...
                    return JsonUtility.ToJson(filter);
                })
                .ToArray();
