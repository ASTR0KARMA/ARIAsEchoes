...
{
    sealed class HierarchyPresenter : NoaDebuggerToolBase, INoaDebuggerTool, INoaDebuggerFloatingTool
    {
        [Header("MainView")]
        [SerializeField]
        ...
        bool _isSelectGameObjectPropertyChanged;
        ...
        public void Init()
        {
            ...
            SettingsEventModel.OnUpdateHierarchySettings -= _OnUpdateHierarchySettings;
            SettingsEventModel.OnUpdateHierarchySettings += _OnUpdateHierarchySettings;
        }
        ...
        {
            _isSelectGameObjectPropertyChanged = true;
        }
        ...
        HierarchyViewLinker _CreateLinker()
        {
            ...
            if (_selectGameObjectEntry != null && _selectGameObjectEntry._gameObject != null)
            {
                ...
                {
                    List<bool> openStateList = new List<bool>();
                    foreach(var component in _componentPropertyList)
                    {
                        openStateList.Add(component._isOpen);
                    }
                    _componentPropertyList = _hierarchyModel.GetGameObjectDetailIEntryList(_selectGameObjectEntry._gameObject);
                    for(var i = 0; i < openStateList.Count; i++)
                    {
                        _componentPropertyList[i]._isOpen = openStateList[i];
                    }
