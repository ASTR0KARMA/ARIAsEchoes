...
{
    sealed class ConsoleLogPresenter : LogPresenterBase<ConsoleLogEntry, ConsoleLogDetail>,
                                       INoaDebuggerTool,
                                       INoaDebuggerOverlayTool
    {
        ...
        public Func<string, List<ConsoleLogEntry>, bool> _onLogDownloadWithLogEntries;

        public UnityAction<List<ConsoleLogEntry>> _onLogSend;

        int _logSerialNumberCounter = 0;

        protected override INoaDownloadCallbacks DownloadCallbacks => NoaConsoleLog.CommonDownloadCallbacks;

        protected override void _Init()
        {
            _overlayPresenter = new ConsoleLogOverlayPresenter(
        ...
        public void ReceiveLogGenerateStackTraceFirstLine(string logString, string stackTrace, UnityEngine.LogType type)
        {
            ...
            {
                string[] splited = stackTrace.Split(NoaDebuggerDefine.NewLine, StringSplitOptions.RemoveEmptyEntries);
        ...
        {
            return _settings.ConsoleLogOverlayShowErrorLogs;
        }

        protected override bool _GetOverlayShowMessageLogsFromNoaDebuggerSettings()
        {
            return _settings.ConsoleLogOverlayShowMessageLogs;
        }

        protected override bool _GetOverlayShowMessageWarningLogsFromNoaDebuggerSettings()
        {
            return  _settings.ConsoleLogOverlayShowWarningLogs;
        }

        protected override int GetLogCapacity()
        {
            var capacity = _settings ? _settings.ConsoleLogCount : NoaDebuggerDefine.DEFAULT_CONSOLE_LOG_COUNT;

            if (capacity < NoaDebuggerDefine.ConsoleLogCountMin || capacity > NoaDebuggerDefine.ConsoleLogCountMax)
            {
                capacity = NoaDebuggerDefine.DEFAULT_CONSOLE_LOG_COUNT;
            }
        ...
        protected override void SetLogPanelInfo(ref LogViewLinker.LogPanelInfo logPanelInfo, ConsoleLogEntry log)
        {
            logPanelInfo.UpdateLogData(logString: log.LogViewString,
                                       stackTrace: log.StackTraceString,
        ...
        protected override string GetFilterSavePrefsKey() => NoaDebuggerPrefsDefine.PrefsKeyIsConsoleLogFilterFlags;

        protected override LogOverlaySettingsDefaultGetter _CreateOverlaySettingsDefaultGetter() => new ConsoleLogOverlaySettingsDefaultGetter();

        protected override string GetOverlayPrefsKey() => NoaDebuggerPrefsDefine.PrefsKeyPrefixConsoleLogOverlaySettings;
        ...
        protected override KeyValueSerializer CreateLogKeyValueSerializer()
        {
            ...
            IEnumerable<ConsoleLogEntry> targetLogs;
            if (NoaConsoleLog.ConsoleLogDownloadCallbacks != null)
            {
                targetLogs = NoaConsoleLog.ConsoleLogDownloadCallbacks.OnBeforeConversion(selectedLogs);
            }
            else
            {
            ...
            foreach (ConsoleLogEntry log in targetLogs)
            {
                ...
                objectParsers.Add(new KeyValueArrayParser.ObjectParser(parsers));
        ...
        {
            return NoaDebuggerSettingsManager.GetCacheSettings<ConsoleLogOverlayRuntimeSettings>();
        }
